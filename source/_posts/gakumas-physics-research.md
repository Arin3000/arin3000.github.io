...
本文研究《学园偶像大师》中花海咲季（HSKI）的头发与服装二级运动：从资产与程序逆向，到解算器制作，再到画面与性能验证。头发主体以原生复现方案为记录基线，整合逆向笔记、制作笔记、版本差异和性能分析；随后对照已导出的布料资料与代码，补充裙摆驱动、碰撞、闭环约束及多服装验证；最后结合公开开发者资料，讨论其他游戏的实时模拟与动画制作方案。

**初期方案、接触修正方案、原生复现方案是本文对三个研究阶段的称呼。** 三版分别回答“怎样让头发动起来”“怎样让指定接触动作更稳定”“怎样尽量还原游戏原本的计算”。

公开下载的代码与证据文本已替换内部迭代号和设备路径。历史清单中的原始文件哈希对应脱敏前的研究归档，不能用于校验公开阅读副本；数值结果没有因此重算。

可复用的头发与裙摆代码已整理为独立的 MIT 仓库：[Open Swing Dynamics](https://github.com/Arin3000/unity-hair-skirt-dynamics)。仓库只包含 Unity 源码与合成示例配置，没有人物模型、贴图、动画或原始角色参数；本文的研究附件继续作为对应阶段的记录。

### 初期方案：建立二级运动

初期方案使用自定义端点弹簧与阻尼，以约 120 Hz 子步更新头发骨段，再配合三个身体碰撞代理、骨长与旋转限制、前发蒙皮采样修正。它解决了导入模型只有主动画、缺少头发动态的问题。当前保留的是初期源码快照，因此本文不会把其中所有常数都视作最终初期方案后期构建的发行实现。

### 接触修正方案：改善展示动作接触

接触修正方案在动态骨基础上加入贴胸、脸部、下巴、袖子和手部处理。它会调整部分网格，也会通过双骨 IK 改变手臂姿态，以改善指定手势下的遮挡与接触。代价是更多 CPU 蒙皮、几何查询、数组与网格更新，以及动作标定工作。现存同名播放器后来经过重建，不能直接当作原始接触修正方案跑分。

### 原生复现方案：依据原始计算与实测输入

原生复现方案按原始指令核对积分、碰撞、旋转、预热和 float32 运算顺序，并接入游戏采集的碰撞列表。当前有 75 段模拟、19 个身体/面部代理、7 个活动链层与 9 份 Quartz 驱动；17/19 项代理身份已确认。它移除了接触修正方案的额外接触制作层，以便检查原生计算结果。已有 3548 组原指令对照和 48 条手部实际采样复算，但还没有完成原游戏同姿态、同调度的端到端轨迹验证。

| 版本 | 主要目标 | 主要方法 | 评价重点 |
|---|---|---|---|
| 初期方案 | 建立可用的二级运动 | 自定义弹簧、子步、简化碰撞与表面修正 | 稳定性与基本观感 |
| 接触修正方案 | 做好指定动作的接触表现 | 动态骨叠加网格修正、手部 IK、袖子烘焙 | 接触质量、姿态变化与制作成本 |
| 原生复现方案 | 尽量恢复游戏原算法 | 原指令对照、原资产参数、实测注册顺序 | 数值一致性、输入完整度与性能边界 |

## 阅读目录

1. [逆向：从资产到原始指令与运行输入](#reverse-engineering)
2. [制作：如何构造并验证解算器](#solver-development)
3. [对比：初期方案、接触修正方案、原生复现方案的方法差异](#version-comparison)
...
**记录基线：原生复现方案，2026-09-15。对象：花海咲季 HSKI、0070 头发、校服身体与基础面部。**
...
数值初核阶段的受控整段积分仍有约 88.59 微米位置误差；数值修正阶段修正执行顺序和 float32 舍入后降至约 2.80 微米，通过原阈值。`余量 / range` 与 `余量 * (1 / range)` 在实数中等价，在逐步单精度舍入下可能不同。这里修正的是运算顺序，没有扩大近乎平行分支的 epsilon，也没有放宽测试阈值。

### 1.7 手部问题揭示了“计算正确、输入仍错”的情况

数值修正阶段第 24 帧中，右手推出头发后，骨长约束、后续胸部碰撞、层末旋转又改变端点。把本地采样输入交给原始指令，会得到很接近的结果。

这能证明“当前输入下的计算接近原版”，却不能证明“游戏也按这个输入计算”。检查发现，本地静态碰撞体沿用 PathID 导出序。于是转向读取游戏正在使用的原生列表。

外部读取在该环境中失败后，沿用已获授权的进程内只读探针。替换诊断 DLL 前确认游戏进程退出并保留备份；采集时不改解算状态。第二轮 在第 11 轮取得匹配目标配置的 19 项数组，早先的零命中只表示当时没有找到通过校验的数据。
...
**基线：原生复现方案。目标：把原资产参数、已核对的原指令行为和实测碰撞顺序接入 Unity，形成可重复验证的播放器。**
...
原生复现方案使用基础面部 1 项与校服身体 18 项原始碰撞数据。动态骨 mask 与静态 mask 按位相与为 0 时跳过；相同骨骼可以有不同类型、不同 mask 的多个代理。
...
原生复现方案的生成器会先验证资产参数与固定采集的参考一致，再匹配参数和句柄关系。已确认部分按游戏排列；仅小腿槽位 4/5 保留旧相对顺序。构建器再校验 19 项的名称、类型与 mask，防止后续导出无意恢复旧顺序。
...
浮点实现方面，数值修正阶段已将关键点积、叉积、四元数乘法和向量旋转按原指令顺序组织，并以 `F32` 固定中间单精度舍入。目的在于避免分支和数值偏移，不是通用性能优化；它有执行开销，也没有取代实测。
...
& 'python' work/build_hski_collision_config.py

# 源码和配置同步到 Unity 项目后，构建会先执行严格检查。
& 'Unity.exe' -batchmode -quit -projectPath './UnityProject/GakumasSample2' -executeMethod HskiHairBuilder.Build -logFile './logs/UnityBuild_native.log'
```

录制使用原生复现方案播放器 的 `-autoWindDemo -handCollisionAudit` 参数；输出到独立原生复现方案目录。重新执行同版录制会重写该版记录，做新实验前先保留基线。

完整结果与证据：[原生复现方案重放报告](/assets/hski-solver-notes/references/8edbea33-native_REGISTRATION_REPLAY_20260915.md.txt)、[源码/录像哈希](/assets/hski-solver-notes/references/1f55310b-manifest.json)。

下一步优先取得原游戏同动画时刻的头颈姿态、根空间输入与动态骨轨迹。现有残差不宜通过加大半径或追加修正迭代“调没”，否则会再次偏离当前的原生复现目标。

<h2 id="version-comparison">三、对比：初期方案、接触修正方案、原生复现方案的方法差异</h2>

**核查日期：2026-09-15；性能分析补充：2026-09-16。比较对象：初期方案、接触修正阶段、当前原生复现方案。**

三者最大的区别在于制作目标和验证依据：初期方案先建立可见的二级摆动；接触修正方案重点处理展示动作中的贴衣、脸/下巴接触和手掌遮挡；原生复现方案根据原始指令与游戏实测数据恢复计算过程。方案推进，不意味着每个画面的所有指标都单调变好。

### 3.1 先说明版本证据，避免拿错文件

| 标签 | 本次采用的主要依据 | 可确认范围 |
|---|---|---|
| 初期方案 | `HskiHairDynamics.prototype_first.txt`、初期方案启动日志、保留录像 | 初期积分和约束设计、运行计数、成片规格 |
| 接触修正阶段 | `work/hski_hair_audit` 中接触修正方案 Builder、动态/接触源码，历史日志与整理脚本 | 当时的制作方案与参数，日志明确记录的运行事实 |
| 原生复现方案 | 固定源码/输出哈希、构建检查、游戏列表归档和实际帧复算 | 当前部署到本地项目的实现与受检结果 |

有两个重要限制：

1. 初期方案源码文件明确叫 `prototype_first`，只能用来说明初期快照；不能把其中每个常数都断言为最终初期方案后期构建的常数。当前 `Player_prototype` 目录还缺少 `HSKI_Showcase_Data`，仅凭 exe 不能重新复核原版运行。
2. **现存 `Player_contact` 已不是干净的原始接触修正方案源码产物。** 读取其 `Assembly-CSharp.dll` 可见 `HskiHairDynamics` 只有构造函数并继承 `ActorAnimationSwingSolver`；后者有 70 个方法。目录中还保留 `build_contact_shared.log`。因此，下文讨论接触修正方案的历史接触方案，不能用当前文件夹名称替代版本身份。核查结果保存在 [类型证据](/assets/hski-solver-notes/evidence/player_contact_current_types.json)。

接触修正方案的 `finalize_contact.py` 还引用了当前缺失的 `record_trace.csv`、`independent_contacts.json`、`live_validation.txt` 等文件。脚本中的断言与说明文本，只能证明它准备检查什么，不能单独证明这些检查现已可复核通过。

### 3.2 总体对照

| 维度 | 初期方案 | 接触修正阶段 | 原生复现方案 |
|---|---|---|---|
...
它也考虑了实际工程问题：采样前恢复基准姿态、头部运动在子步内插值、瞬移/大转角触发重置、碰撞后修正速度，以及用前发蒙皮采样做贴衣约束。因此初期方案不是简单加一段正弦摆动，但它的约束和公式还以本地稳定与观感为依据。

与原生复现方案最直接的区别，是原生复现方案不再把上述经典端点弹簧当作原算法。原版 `(childDefault-current)*(1-damping)^2`、投影前的 `speed` 保存位置，以及先求旋转再投影，都已有机器码依据。

来源：[初期源码](/assets/hski-solver-notes/evidence/prototype_first_HskiHairDynamics.txt)、[初期方案启动记录](/assets/hski-solver-notes/references/6e90772a-smoke_prototype.log.txt)。

### 3.4 接触修正方案：把指定接触动作做得更稳定

#### 动态结构的变化

保留的 接触修正阶段源码让非前发维护独立的头部滞后位置/旋转，减少已修正父节点对下游输入的反馈。前长卷发另走 `StepFrontChains`：质量为零的根留在动画姿态；其他节点使用头部角速度、过滤后的线速度与微风构造目标角，再经临界阻尼角弹簧平滑。
...
接触修正方案 Builder 配置了贴胸参考、面部投影弯曲、下巴三角面近邻约束、手部接触与袖子修正；身体接触后另发布网格，保证显示姿态跟上接触计算。
...
来源：[接触修正阶段动态源码](/assets/hski-solver-notes/evidence/contact_stage_HskiHairDynamics.cs.txt)、[接触修正方案 Builder](/assets/hski-solver-notes/evidence/contact_stage_HskiHairBuilder.cs.txt)、[手部接触源码](/assets/hski-solver-notes/evidence/contact_stage_HskiHandHairContact.cs.txt)。

### 3.5 原生复现方案：恢复原数据的意义和执行顺序

原生复现方案沿用数值修正阶段已通过对照的计算核，并加入游戏实测静态列表。与前两代相比，变化包括：
...
5. **场景清理**：原生复现方案 Builder 移除旧贴胸网格、手部 IK 压发、脸/下巴和袖子额外修正，避免这些方案改变原生计算结果的观察。
...
### 3.6 为什么不能简单说原生复现方案 “穿模更少”

接触修正方案常用脸部/下巴网格的局部有符号距离、手部投影接触残差；原生复现方案本次主要记录动态球与原始静态胶囊的代理重叠。前者可能还伴随身体 IK 和头发网格变形，后者保留原生骨段约束。它们测量对象、阶段和基准姿态不同。

所以，接触修正方案某项记录“小于 1 mm”，不能与原生复现方案的“22.56 mm 代理重叠”直接排大小。现存接触修正方案 `validation.txt` 本身也写明其局部诊断不保证完整网格不相交；而整理脚本引用的部分独立检查结果目前缺失，不能补写成已确认通过。

当前可直接比较的量来自统一审计口径的 **数值修正阶段 → 原生复现方案**：

| 同一采样口径 | 数值修正阶段 | 原生复现方案 |
|---|---:|---:|
...
保留的初期方案、接触修正方案视频均为 1920×1080、30 fps、301 帧；原生复现方案当前正背面视频分别为 1280×720、30 fps、301 帧。输出帧率是编码规格，不是运行时性能。

公平比较应统一：模型/动画版本、采样时刻、镜头、风、预热和重置条件、是否启用手部 IK/网格修正，以及测量阶段。跨分辨率、跨组件堆栈的整体 FPS，不能写成纯解算器加速倍数。现有资料不足以给出原生复现方案相比初期方案/接触修正方案的严格性能百分比。

#### 结合源码与已有计时的判断

| 方案 | 性能主要受什么影响 | 当前可下的结论 |
|---|---|---|
| 初期方案 | 约 120 Hz 子步、三轮代理投影、前发蒙皮采样和躯干表面刷新 | 3 个代理不等于整套方案必然最轻；日志中的 fps 是测试输入，缺少可比实测 |
| 接触修正阶段 | BakeMesh、多个网格接触模块、数组复制、法线/切线与身体网格发布 | 最值得先排查 CPU 网格路径；不能仅凭源码断言实测最慢 |
| 原生复现方案 | 19 个代理、活动 Chain、Quartz、严格 F32 调用、诊断及重置预热 | 普通区间较短，动作首帧尖峰值得定位；移除了旧网格修正，但未测单独解算耗时 |

接触修正方案的袖子烘焙库约 40.08 MiB（原始 JSON 文件大小）。成功加载时跳过实时五轮约束，仍逐帧插值、分配数组并更新网格；它用加载和内存成本换取部分计算量，不能将实时五轮算法算作该分支每帧负担。

本次对原生复现方案原始 CSV 重算：去掉开头 10 行后共 **4240 个区间，平均 2.357 ms，中位 2.299 ms，P95 2.607 ms，P99 2.810 ms**。两个超过 16.667 ms 的区间为 20.475 / 21.095 ms，分别对应动作 1、2 首帧。每次动作切换都会请求重置并触发预热，但现有计时不能把全部尖峰归因于预热。
...
结合当前严格还原目标，继续原生复现方案更合适：先计时定位重置尖峰，再评估诊断开销、重复变换和缓冲区复用。所有优化都应保留注册顺序与数值语义，并通过原指令对照及真实帧复算。**当前没有三种方案的性能加速倍数，也没有移动端、多角色或低功耗结论。**

完整的逐动作数据、帧时图、源码成本分析和后续测试方法见 [第四部分](#performance)。

录像入口：[初期方案](/assets/hski-solver-notes/media/a6e17372.mp4)、[接触修正方案](/assets/hski-solver-notes/media/43d8e3e1.mp4)、[原生复现方案正面](/assets/hski-solver-notes/media/48812927.mp4)。

### 3.8 三代各自留下了什么

- **初期方案**：验证原骨架可以承载二级运动，建立姿态恢复、子步、重置、约束和画面对照的基础。
- **接触修正方案**：探索了实际网格接触、局部变形、手部 IK 与显示姿态同步；同时暴露了动作标定和多层修正对结果的影响。
- **原生复现方案**：把字段解释、计算过程和输入顺序变成可检验的对象，能够说明误差来源，也能明确哪些地方仍未确认。

如果目标是当前这条“严格恢复游戏解算”的工作，应继续沿原生复现方案取得原游戏同姿态轨迹。接触修正方案的网格与 IK 方案可以作为另一种制作手段保留，但一旦重新启用，就要明确最终画面还包含这些额外处理，不能全部归因于原生解算器。

下一轮的明确缺口是两条小腿身份、完整游戏姿态/调度输入和剩余手部残差。不能仅因进入原生复现方案，就把这些未完成项隐藏起来。

配套阅读：[逆向笔记](#reverse-engineering)、[制作笔记](#solver-development)、[原生复现方案实测重放报告](/assets/hski-solver-notes/references/8edbea33-native_REGISTRATION_REPLAY_20260915.md.txt)。
...
**分析日期：2026-09-16。本次重新统计已有原生复现方案采样，并核对三版源码；没有重新运行三版播放器。**

结合还原目标与性能，当前适合继续推进原生复现方案。它移除了接触修正方案的多层网格接触修正，已有记录也显示当前场景大多数采样区间较短；但还不能断言它比初期方案或原始接触修正方案快多少。初期方案的成本不能只按 3 个碰撞体判断，接触修正方案的成本不能只按 75 段骨骼判断，原生复现方案的 424 FPS 也不能解释成解算器单独运行速度。

本文将 **实测**、**源码确定的行为**、**待计时验证的推断** 分开说明。版本身份限制见 [第三部分](#version-comparison)。

### 4.1 三版性能结论速查

| 项目 | 初期方案快照 | 接触修正阶段 | 原生复现方案当前方案 |
|---|---|---|---|
...
[记录器快照](/assets/hski-solver-notes/evidence/perf_native_HskiAutoWindDemoRecorder.cs.txt) 中，实时测试先记录时间，再执行：
...
![原生复现方案已有记录的帧时分布](/assets/hski-solver-notes/evidence/native_frame_intervals.svg)
...
原生复现方案的 `recordingThroughputFps=26.53` 来自录像流程，包含额外相机渲染、ReadPixels、PNG 编码、管道写入和 ffmpeg 等开销。正背面分别写同一组统计字段，因此最终 JSON 的 301 帧 / 11.346 秒对应最后的背面录制。实时基准不执行逐帧截图和手部 JSONL 审计；这些属于后续确定性录像阶段。

初期方案、接触修正方案保留视频为 1080p、30 fps；原生复现方案视频为 720p、30 fps。三者的 30 fps 都是编码规格。初期方案/接触修正方案验证日志中的 30/60/120 fps 还可能是 `Step(1/fps)` 的测试输入，不能当作跑分。

此外，当前 Player_prototype 缺少数据目录，当前 Player_contact 的程序集已包含后来的共享解算器。不能直接启动这两个目录，就把结果命名为原版初期方案/接触修正方案。接触修正方案的 live_trace 等部分原始文件缺失；其 LiveAudit 源码还会在多轮测试中截图，未来若恢复记录，也需标记截图影响。

工作区另有晨间教室旧/新核心路径约 3.23 倍的性能报告，但对象是另一组角色更新与渲染路径 A/B，**不能借用为本次三代头发解算器的加速倍数**。

### 4.3 从源码解释开销差异

#### 3.1 初期方案：代理少，子步和表面处理仍要付费

初期方案代码使用 `Clamp(CeilToInt(delta*120),1,12)`。名义 30/60/120 Hz 输入时，通常对应 4/2/1 个子步；具体以浮点取整和实际 delta 为准。每子步按节点做弹簧、速度、旋转限制和三轮代理碰撞。

取 75 个有效节点、3 个代理为示例，30 Hz 时投影循环有约 `4 × 75 × 3 × 3 = 2700` 次候选访问。这个数字只是循环次数，不是命中数、CPU 指令数或耗时；未计额外残差检查、Transform 调用及表面处理。

初期方案还调用躯干表面刷新，进行最多六轮前发节点的蒙皮采样与旋转修正，再审计前发表面。采样点 World() 会按骨权重重新计算位置。因此，“只有 75 段、3 个碰撞体，所以一定最轻”不成立。

**推断：** 如果只看基础代理动力学，初期方案的公式和代理输入较简单；但低显示帧率下每帧子步增多，前发采样密度也会影响总成本。整套效果是否最快仍需计时。来源：[初期快照](/assets/hski-solver-notes/evidence/prototype_first_HskiHairDynamics.txt)。

#### 3.2 接触修正方案：成本主要扩展到了可见网格

[ContactMesh](/assets/hski-solver-notes/evidence/perf_contact_HskiHairContactMesh.cs.txt) 对每个绑定执行 BakeMesh、读取顶点/法线/切线、变换到世界空间，再处理贴胸、顶点及三角形内部采样，最后更新网格。之后还依次调用袖子、面部、下巴、手部修正。
...
烘焙路径也不是只查一个数字：[ApplyClip](/assets/hski-solver-notes/evidence/perf_contact_HskiHairSleeveContact.cs.txt) 每次从帧列表中查找相邻帧，按绑定 Clone 原位置、新建偏移数组，插值后调用 UpdateMesh；后者还新建两份法线累计数组、遍历三角形并更新网格。**它省掉部分实时几何求解，但没有省掉逐帧网格变形与分配。**

因此，源码支持“接触修正方案存在更多 CPU 网格处理和 GC 压力来源”的判断，尚不支持“接触修正方案实测最慢”或“GC 每帧 X MB”。

#### 3.3 原生复现方案：原始代理更多，执行结构更集中

[当前核心快照](/assets/hski-solver-notes/evidence/perf_native_ActorAnimationSwingSolver.cs.txt) 按深度处理骨段，处理活动链层并写回受约束姿态。75 段、19 个代理、7 个活动链层、9 份 Quartz 构成当前输入；正常 Unity 角色渲染仍在。
...
- **碰撞诊断也付费。** CheckDynamicCollision 先遍历代理投影，再遍历代理记录残差。仅普通节点的两轮候选遍历，按 75×19×2 为 2850 次；仍未计 Chain、预热和其他指标，并有 mask 筛选。它与初期方案的 2700 不是等价几何工作量。
...
原生复现方案移除旧网格接触路径后，可优化对象更集中，但 F32、Transform、残差与链层仍有工作量；目前不能把约 2.3 ms 全算给这些操作，也不能推导一帧能容纳多少角色。

### 4.4 综合性能与还原质量，应怎样取舍

| 目标 | 判断 | 需要保留的边界 |
|---|---|---|
| 严格恢复游戏解算 | 继续原生复现方案，并补齐原游戏姿态/调度证据 | 保留原参数、列表顺序、浮点语义与重置行为 |
| 指定展示动作的局部贴合 | 接触修正方案可作为独立的接触制作方案 | 额外 IK 与网格修正改变姿态和结果；收益与额外成本一起评价 |
| 理解早期弹簧及子步行为 | 保留初期方案为基线 | 不能把当前快照宣称为最终初期方案的后期构建，也不能以简单代理数作性能排名 |
| 多角色、移动端或低功耗 | 现有记录不够下结论 | 需同平台数据、长时间运行及锁帧功耗，不按 424 FPS 线性外推 |

对当前项目，优先级建议是 **原生复现方案的重置帧定位 → 诊断与重复变换的计时 → 保持数值一致的实现优化**。没有证据表明为了性能需要退回初期方案或重新加入整套接触修正方案网格修正。
...
4. **若继续维护接触修正方案，先复用缓冲区。** 处理 Clone/new、重复法线累计和烘焙帧查找；可以探索合并网格发布，但需保留每个接触模块读到的正确中间几何。不能把 IK 前后两次身体蒙皮盲目共用。
...
- **完整方案测试：** 相同硬件、电源条件、资源、镜头、渲染规格、动作时刻、风与重置事件，保留各方案必要功能，衡量最终画面及总体开销。接触修正方案的接触改善和姿态改变也要一起展示。
...
旧方案保留自己的子步规则，原生复现方案保留自己的 Step 规则；在同一固定输入频率下比较，并记录实际内部迭代次数。不同公式造成的运动差异应单独评估，不能通过偷偷改步进让曲线或跑分更好看。
...
- [本次重算 JSON](/assets/hski-solver-notes/evidence/native_performance_reanalysis.json)、[重算脚本](/assets/hski-solver-notes/evidence/analyze_performance.py.txt)。脚本读取现存 CSV，输出统计与图，不启动游戏。
- [证据清单及源码快照哈希](/assets/hski-solver-notes/evidence/notes_manifest.json)。历史接触组件快照说明源码路径行为，不保证其每行都与已缺失的原始发行程序集一致。

**结论：原生复现方案的当前场景区间整体较短，动作首帧约 20–21 ms 是明确的排查入口；接触修正方案的多层 CPU 网格处理是明确的潜在成本来源；初期方案也包含子步与表面约束。三版谁快多少，仍缺同条件、身份明确的实测。**

<h2 id="cloth-research">五、布料研究：裙根驱动、动态骨链与服装验证</h2>

本节补充原生复现方案头发基线之后的裙摆研究，依据本地 `hski_skirt_motion_audit`、校服修正和衣柜扩展的源码与结果记录。**这是一组后续布料实现与验证，不代表前文原生复现方案头发测试已经覆盖全部服装。** 本次核对的四份运行源码与衣柜验证保存的 SHA-256 清单一致；数值和几何结果来自已有采样，并非本次重新运行游戏所得。
...
### 5.7 与接触修正方案袖子方案、原生复现方案头发方案的关系及性能

| 路径 | 被更新的对象 | 主要工作 | 已知边界 |
|---|---|---|---|
| 接触修正方案袖口／服装修正 | 网格顶点与部分骨骼 | 接触标定、CPU 蒙皮／几何处理；已有袖子缓存的插值与网格更新 | 属于特定动作的制作修正，不能据此推断游戏原生布料算法 |
| 原生复现方案头发基线 | 发骨端点和旋转 | ActorSwing 积分、碰撞、层间传递及 Quartz 驱动 | 头发测试对象没有启用闭环平滑，不能替代布料验证 |
...
正常播放的裙摆路径主要改骨骼；这里用于验收的 `BakeMesh`、顶点写盘、截图和 Python 三角形检查属于诊断流程。审计模式设置 `captureDeltaTime=1/60` 只是固定采样时间，**不能当作实测稳定 60 FPS**。目前没有与前文初期方案 / 接触修正方案 / 原生复现方案使用相同场景、计时范围的布料耗时对照，因此不填造布料毫秒数，也不把前文原生复现方案的帧区间耗时当作裙摆 `Step` 成本。
...
对本研究的启发是：**实时模拟与人工控制可以共同存在。** 实时层负责对输入作出响应，作者制作的目标姿态负责维护角色剪影和特定接触。Guide Motion 与接触修正方案都体现了“为最终画面加入额外控制”的思路，但不能把两者具体实现画等号，更不能据此给原生复现方案追加修正后仍声称是同一原生算法。
...
对于“还原《学园偶像大师》”这条目标，原生复现方案仍应保留原计算核、参数和注册顺序，继续核对原游戏姿态与调度输入。不能因为引导动画能改善某个画面，就把它偷偷加入对照基线。

如果目标转向可交付的角色表现，则可另建明确标注的混合分支：保留原生复现方案作为参考，在容易穿插的手部动作、坐姿或特写中试验引导动画与局部实时响应。评测应把“更符合原游戏计算”和“更符合指定镜头要求”列为两个维度。

接触修正方案已经提供了一个本地例子：其袖子烘焙数据、网格变形与手部 IK 证明额外制作层能够与实时骨段计算共同存在，同时也带来存储、采样与网格更新负担。它与《原神》或《Relink》的具体架构不能直接等同，但可以帮助设计具有明确边界的对照实验。
